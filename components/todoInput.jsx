...
export default function InputInline({ date, todo, setTodo, setTodos }) {
  return (
    <Field orientation="horizontal" className="w-full max-w-3xl px-4 mb-8 h-auto sm:h-10 z-10 flex-col sm:flex-row items-stretch sm:items-center">
      ...
      <Button suppressHydrationWarning onClick={() => {
        if (todo.title.trim() !== "" && todo.desp.trim() !== "") {
          setTodos((prevTodos) => {
            if(todo.id === null) {
              return [...prevTodos, { ...todo, done: false, id: Date.now(), createdAt: new Date(), completionDate: date }];
            } else {
              return prevTodos.map((t) => t.id === todo.id ? { ...todo, title: todo.title, desp: todo.desp, completionDate: date } : t);
            }
          });
          setTodo({ title: "", desp: "", done: false, id: null, createdAt: null, completionDate: null });
        }
